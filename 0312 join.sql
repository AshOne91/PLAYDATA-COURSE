...
-- scores 테이블에서 전체평균보다 높은 학생이름과 점수
with total_avg as (
	select
    avg(score) as all_avg
from student_db3.exam_scores
)
select
se.name, se.score
from student_db3.exam_scores se, total_avg ta
where se.score > ta.all_avg
;
...
where teacher = '김선생' and student_score > 80
;
